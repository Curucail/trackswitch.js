...
const interactiveScriptPattern =
	/trackswitch-interactive(?:\.min)?\.js(?:\?.*)?$/;
...
function resampleFeatureMatrix(
	...
	const targetFrameCount = Math.max(
		1,
		Math.round((matrix.frameCount / sourceFrameRate) * targetFrameRate),
	);
	...
	for (
		...
		targetFrameIndex < targetFrameCount;
		targetFrameIndex += 1
	) {
		...
		const nextSourceFrameIndex = Math.min(
			sourceFrameIndex + 1,
			matrix.frameCount - 1,
		);
		const interpolation = Math.min(
			Math.max(sourcePosition - sourceFrameIndex, 0),
			1,
		);

		for (let binIndex = 0; binIndex < matrix.binCount; binIndex += 1) {
			const currentValue =
				sourceData[sourceFrameIndex * matrix.binCount + binIndex];
			const nextValue =
				sourceData[nextSourceFrameIndex * matrix.binCount + binIndex];
...
export function cloneBasicPitchFeatureSet(
	features: BasicPitchFeatureSet,
): BasicPitchFeatureSet {
	return {
...
export function resolveBasicPitchModelUrl(workerUrl?: string): string {
	...
	if (baseUrl) {
		return new URL(
			BASIC_PITCH_MODEL_RELATIVE_URL,
...
export async function ensureBasicPitchFeatures(
	...
	if (file.type !== "audio" || !file.pcmData) {
		throw new Error(
			"Basic Pitch feature extraction requires an audio file with decoded PCM data.",
		);
	}
