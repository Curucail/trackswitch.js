...
function validateVariantConfig(
	...
	if (variant === "default") {
		...
		if (resolvedUi?.some(isAlignmentWaveform)) {
			throw new Error(
				"Invalid default player configuration: aligned waveform options require the alignment player variant.",
			);
		}
		return;
	}
	...
	if (!init.alignment) {
		throw new Error(
			"Invalid alignment player configuration: alignment config is required.",
		);
	}
	...
	tracks.forEach((track) => {
		...
		if (typeof column !== "string" || column.trim().length === 0) {
			throw new Error(
				"Invalid alignment player configuration: each track requires alignment.column.",
			);
		}
	});
}
...
export function normalizeTrackSwitchConfig(
	...
	const usesExclusiveSoloMode =
		normalizedFeatures.exclusiveSolo || options.variant === "alignment";
