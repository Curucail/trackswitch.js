...
export function getInlineConfigScripts(element: HTMLElement): HTMLScriptElement[] {
    return Array.from(element.children).filter(function(child): child is HTMLScriptElement {
        return child instanceof HTMLScriptElement
...
export async function loadElementConfig<TConfig>(
    ...
    if (configSrc && inlineConfigScripts.length > 0) {
        throw new Error('TrackSwitch config error: use either config-src or inline JSON, not both.');
    }

    if (inlineConfigScripts.length > 1) {
        throw new Error('TrackSwitch config error: expected exactly one inline JSON config script.');
    }
    ...
    if (configSrc) {
        ...
        try {
            ...
            throw new Error('TrackSwitch config error: failed to load config-src "' + configSrc + '".');
        }
        ...
        try {
            ...
            throw new Error('TrackSwitch config error: invalid JSON from config-src "' + configSrc + '".');
        }
    }
