...
export function renderMarkerLayers(
	...
	controller.visibleMarkerSetIds = controller.renderer.renderTimelineMarkers({
		...
		audibleMarkerSetIds: new Set(
			getAudibleMarkerSets(controller).map((resolved) => String(resolved.id)),
		),
...
function getVisibleMarkerSets(
	...
	return Array.from(controller.markerSets.entries())
		.filter(([setId]) => controller.visibleMarkerSetIds.has(setId))
...
function getNavigationMarkerTimes(
	...
	const times: number[] = [];
	getVisibleMarkerSets(controller).forEach((resolved) => {
