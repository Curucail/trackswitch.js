    ...
    public buildSheetMusicMeasureMaps(measureColumn: string, source: string): Promise<SheetMusicMeasureMapsByAxis> {
        ...
        if (!this.alignmentConfig) {
            return Promise.reject(
                new Error('Sheet music measure sync requires init.alignment when sheetMusic.measureColumn is set.')
            );
        }

        return this.loadAlignmentCsv().then((parsedCsv) => {
            const referenceTimeColumn = this.resolveReferenceTimeColumn(this.alignmentConfig as TrackAlignmentConfig);
            ...
            const referenceTimeColumnSync = this.resolveReferenceTimeColumnSync(this.alignmentConfig as TrackAlignmentConfig);
