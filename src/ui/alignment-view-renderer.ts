...
type WarpingMatrixPlotState = NonNullable<WarpingMatrixHostMetadata['matrixPlot']>;
type WarpingTempoPlotState = NonNullable<WarpingMatrixHostMetadata['tempoPlot']>;
type WarpingMatrixPathSeriesData = NonNullable<WarpingMatrixHostMetadata['matrixDataCache']>['byColumn'] extends Map<string, infer T> ? T : never;
type WarpingMatrixMatrixData = NonNullable<WarpingMatrixHostMetadata['matrixDataCache']>;
type WarpingMatrixTempoSeriesData = NonNullable<WarpingMatrixHostMetadata['tempoDataCache']>['byColumn'] extends Map<string, infer T> ? T : never;
type WarpingMatrixTempoData = NonNullable<WarpingMatrixHostMetadata['tempoDataCache']>;
...
export class AlignmentViewRenderer extends ViewRenderer {
    ...
    public resolveWarpingMatrixColumnColor(columnKey: string, columnOrder: string[]): string {
        return viewRendererWarping.resolveWarpingMatrixColumnColor(this, columnKey, columnOrder);
    }
    ...
    public createWarpingMatrixPlotState(plotHost: HTMLElement, width: number, height: number): WarpingMatrixPlotState {
        return viewRendererWarping.createWarpingMatrixPlotState(this, plotHost, width, height);
    }

    public createWarpingTempoPlotState(plotHost: HTMLElement, width: number, height: number): WarpingTempoPlotState {
        return viewRendererWarping.createWarpingTempoPlotState(this, plotHost, width, height);
    }

    public applyWarpingMatrixPlotDimensions(plot: WarpingMatrixPlotState, width: number, height: number): void {
        return viewRendererWarping.applyWarpingMatrixPlotDimensions(this, plot, width, height);
    }

    public applyWarpingTempoPlotDimensions(plot: WarpingTempoPlotState, width: number, height: number): void {
    ...
    public applyWarpingMatrixContext(host: WarpingMatrixHostMetadata, context: WarpingMatrixRenderContext): void {
    ...
    public resolveCenteredWarpingWindow(center: number, windowSeconds: number, maxTime: number): [number, number] {
        return viewRendererWarping.resolveCenteredWarpingWindow(this, center, windowSeconds, maxTime);
    }
    ...
    public interpolateWarpingTrackTime(points: Array<{ referenceTime: number; trackTime: number }>, referenceTime: number): number {
        return viewRendererWarping.interpolateWarpingTrackTime(this, points, referenceTime);
    }

    public interpolateWarpingReferenceTime(pointsByTrackTime: Array<{ referenceTime: number; trackTime: number }>, trackTime: number): number {
