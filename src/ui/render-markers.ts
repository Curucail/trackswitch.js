...
export interface MarkerRenderData {
	...
	/** Marker set ids currently reachable by prev/next navigation — see getAudibleMarkerSets. */
	audibleMarkerSetIds: ReadonlySet<string>;
...
function resolveMarkerPlayerTime(
	...
	return data.projection.projectMarker(marker, data.playerTimeline);
}
...
function renderConfiguredLayers(
	...
	layers.forEach((layer) => {
		...
		if (restrictToAudible && !data.audibleMarkerSetIds.has(layer.set)) {
			return;
		}

		visibleSetIds.add(layer.set);
		renderMarkerLayer(seekWrap, layer, resolvedSet.markerSet.markers, data);
	});
}

/**
 * Renders marker layers on every seek surface and reports which marker sets
 * ended up actually shown somewhere. Navigation (prev/next) is scoped to this
 * result rather than to audible tracks, so a marker set rendered on a
